...
public :
	Machine( string nom_machine_, int prix_machine_, int n_serie_machine_ ) : nom_machine(nom_machine_), prix_machine(prix_machine_), n_serie_machine(n_serie_machine_) {}


        Machine(const json& data): nom_machine(data["nom_machine"].get<string>()),
...
public :
	Objet( string nom_objet_, int prix_objet_) : nom_objet(nom_objet_), prix_objet(prix_objet_) {}


        Objet(const json& data): nom_objet(data["nom_objet"].get<string>()),
...
public :
	Usine(int machine_) : machine(machine_) {p_machine = make_unique<Machine>(machine);}
...
public :
	QuantiteRessource(int quantite_, int ressource_ ) : quantite(quantite_), ressource(ressource_) {
	    p_ressource = make_unique<Ressource>(ressource);
	}

        QuantiteRessource(const json& data): quantite(data["quantite"]),
	...
	{
		cpr::Response r = cpr::Get(cpr::Url{"http://127.0.0.1:8000/quantiteressource/" + to_string(id) + "/"});
...
public :
	Stock( int usine_, int objet_, int nombre_ ) : usine(nom_local_), objet(ville_), nombre(surface_local_) {
	    p_objet = make_unique<Objet>(objet);
			p_usine = make_unique<Usine>(usine);
	}

        Stock(const json& data): usine(data["usine"]),
        objet(data["objet"]),
        nombre(data["nombre"]) {
					p_objet = make_unique<Objet>(objet);
					p_usine = make_unique<Usine>(usine);
...
public :
	Etape( string nom_etape_, int quantite_ressource_, int duree_, int machine_, int etape_suivante_ ) : nom_etape(nom_etape_), quantite_ressource(quantite_ressource_), duree(duree_), machine(machine_), etape_suivante(etape_suivante_) {
	    p_quantite_ressource = make_unique<QuantiteRessource>(quantite_ressource);
			p_machine = make_unique<Machine>(machine);
			p_etape_suivante = make_unique<Etape>(etape_suivante);
	}

        Etape(const json& data): nom_etape(data["nom_etape"]),
